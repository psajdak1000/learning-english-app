...
export default function StartingPage({
    ...
    const openChatbot = () => {
        // 2. TERAZ URUCHAMIAMY PRZEKIEROWANIE ZAMIAST ALERTU
        onChatClick?.();
    };
        ...
        <div className="starting-page">
            <header className="header">
                ...
                <div className="header-actions">
                    {user ? (
                        <>
                            <span className="user-greeting">Witaj, <strong>{user.username}</strong>!</span>
                            <button className="logout-btn" onClick={onLogout}>Wyloguj się</button>
                        </>
                    ) : (
                        <button className="login-btn" onClick={handleLoginClick}>Zaloguj się</button>
                    )}
                </div>
            </header>
